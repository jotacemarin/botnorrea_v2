...
// index.ts
var botnorrea_v2_exports = {};
__export(botnorrea_v2_exports, {
  dummy: () => dummy,
  handler: () => handler
});
module.exports = __toCommonJS(botnorrea_v2_exports);

// src/handler/index.ts
var import_http_status = __toESM(require_lib());
var handler = async (event, _context, callback) => {
  const body = JSON.stringify({
    message: "Handler endpoint: Serverless v3.0! Your function executed successfully!",
    input: event
  });
  return callback(null, {
    statusCode: import_http_status.OK,
    body
  });
};

// src/dummy/index.ts
var import_http_status2 = __toESM(require_lib());
var dummy = async (event, _context, callback) => {
  const body = JSON.stringify({
    message: "Dummy endpoint: Serverless v3.0! Your function executed successfully!",
    input: event
  });
  return callback(null, {
    statusCode: import_http_status2.OK,
    body
  });
};
